def calculate_amount():
    ...
    """
    # Diccionario maestro: { "ID": (Nombre, {Cuotas: TasaMensual}) }
    card_data = {
    ...
    try:
        ...
        if pay_form == "1":
            ...
            print("Action: 10% debit discount applied.")
        elif pay_form == "2":
            total_amount = amount_user * 0.87
            print("Action: 13% cash discount applied.")
        else:
            # Lógica Unificada de Tarjetas
            while True:
                print("\nSelect the card:")
                for key, (name, _) in card_data.items():
                    print(f"{key} - {name}")
                card_choice = input("Option: ")
            ...
            # Fórmula: Total = Monto + (Monto * TasaMensual% * CantidadCuotas)
            ...
            print("="*35)
            return  # Finalizamos aquí para evitar el print doble
